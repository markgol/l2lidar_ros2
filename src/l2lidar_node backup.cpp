...
//		A x86_64 imeplentation wukk be done after the RPI5 version
//		completed.
//
...
#include "L2lidar.h"   // your class
#include "PCpoint.h"

//++++++++++++++++++++++++++++++++++++++++
// L2lidarNode class
//++++++++++++++++++++++++++++++++++++++++
class L2LidarNode : public rclcpp::Node
{
    Q_OBJECT
...
//---------------------------------------------------------------------
    L2LidarNode(int argc, char **argv)
        : Node("l2lidar_node"),
          qt_app_(argc, argv)
    {
        ...
        RCLCPP_INFO(get_logger(), "L2Lidar ROS2 node started (Qt 6.10 runtime)");
    }
...
//---------------------------------------------------------------------
    void onImuReceived()
    {
        ...
        msg.orientation.x = imu_packet.quaternion[0];
        msg.orientation.y = imu_packet.quaternion[1];
        msg.orientation.z = imu_packet.quaternion[2];
        msg.orientation.w = imu_packet.quaternion[3];

        msg.angular_velocity.x = imu_packet.angular_velocity[0];
        msg.angular_velocity.y = imu_packet.angular_velocity[1];
        msg.angular_velocity.z = imu_packet.angular_velocity[2];

        msg.linear_acceleration.x = imu_packet.linear_acceleration[0];
        msg.linear_acceleration.y = imu_packet.linear_acceleration[1];
        msg.linear_acceleration.z = imu_packet.linear_acceleration[2];
		// ??? missing timestamp
		// imu_packet.info.stamp.sec
		// imu_packet.info.stamp.nsec
		// need to turn into seconds
